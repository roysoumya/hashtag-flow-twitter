...
L=len(intervals)
p=len(content)/7
counts=[[0 for x in range(L-1)] for x in range(p)]
testcounts=[[0,0]  for x in range(p)]
...
#for n in range(1):
	...
	for i in range(len(k_series)):
		...
		"""
		kernel = tf.exp(omega*current_t/(current_kh*current_k)) 
		next_h = beta*kernel + gamma*current_h + bh
		...
		kernel=[0]*batch_size
		next_h=[0]*batch_size
		next_kh=[0]*batch_size
		kernel[0]=tf.exp((omega[0]*current_t[0]/(current_kh[-1]*current_k[0]))/expmod) 
		next_h[0]=beta[0]*kernel[0]+gamma[0]*current_h[-1]+bh[0]
		next_kh[0] = tf.exp(v[n][0]*next_h[0])
		for j in range(1, batch_size):
			kernel[j]=tf.exp((omega[j]*current_t[j]/(next_kh[j-1]*current_k[j]))/expmod)
			next_h[j]=beta[j]*kernel[j]+gamma[j]*next_h[j-1]+bh[j]
			next_kh[j] = tf.exp((v[n][j]*next_h[j])/expmod)
